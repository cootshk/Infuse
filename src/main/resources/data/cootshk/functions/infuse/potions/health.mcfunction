...
execute if entity @s[gamemode=!creative] if score @s Health matches 1 run loot give @s loot cootshk:infuse/health
execute if entity @s[gamemode=!creative] if score @s Health matches 1 run clear @s glass_bottle 1
execute if score @s Health matches 1 run return 1
scoreboard players set @s Health 1
tellraw @s [{"text":"[","color":"gold"},{"text":"Infuse","color":"dark_aqua"},{"text":"] ","color":"gold"},{"text":"You have drank the Health Boost effect!","color":"yellow"}]